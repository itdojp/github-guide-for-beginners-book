...
- **四半期に1回**（例：3か月に1回）、次の観点で棚卸しします。
  ...
  - Agents / MCP 等の AI 連携機能の導線・名称が変わっていないか（記載がある場合）
  - エージェント運用に必要なテンプレ（Issue/PR）の項目が妥当か（受入基準/検証/禁止事項）
  - AI 機能や課金体系（例：premium requests 等）の説明が陳腐化していないか（記載がある場合）
...
- [ ] Agents / MCP / 料金等の説明がある場合、現行 UI・用語・運用と矛盾していない
