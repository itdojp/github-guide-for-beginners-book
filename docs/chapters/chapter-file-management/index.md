...
新しいファイルを追加するには、画面上部の「Add file」メニュー（または同等の項目）を開きます。すると、以下の選択肢が表示されます：
...
ファイルをドロップすると、アップロードの進行状況が表示されます。完了すると、「Commit changes」という画面が表示されます。これは「この変更を記録として残しますか？」という確認画面です。詳しくは5.2節で説明します。
...
1. 削除したいファイルを開く
2. 画面上部のメニューから「Delete file」を選択する（UI によっては「…」メニュー内にあります）
3. 確認画面で削除理由を入力（任意）
...
GitHub Desktopというアプリケーションを使うと、最新のcommitメッセージを修正できます（第6章で詳しく説明します）。

---

## 5.4 間違ったcommitを取り消す方法
...
**A:** はい、可能です。Revertした変更を再度有効にしたい場合は、Revert の commit を Revert してください。

**Q: 古いcommitだけをピンポイントでRevertできますか？**  
**A:** 可能ですが、その後の commit で同じ部分が変更されている場合、コンフリクトが発生する可能性があります。

![コンフリクト解決方法]({{ '/assets/images/diagrams/chapter04/07_conflict_resolution.svg' | relative_url }})

**Q: Web画面以外でRevertする方法はありますか？**  
**A:** はい、GitHub Desktop や Git コマンドを使用しても同様の操作が可能です。
