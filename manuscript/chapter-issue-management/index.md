...
### AI/エージェント向けに Issue を「実行仕様」として書く

Issue は「やることメモ」としても運用できますが、AI 支援やエージェントが Pull Request を作る前提では、**Issue を実行可能な仕様（実行仕様）** として書くのが安全です。情報が不足すると、次のような問題が起きやすくなります。

- 期待結果が曖昧で、差分が過剰になる（関係ない変更が混ざる）
- テスト方法が不明で、レビュー基準が揃わない
- 触ってはいけない設定や機密情報に誤って触れる
...
**書き方のポイント**
- 受入基準は「誰が読んでも同じ判定」になる粒度で書く
- 制約・変更禁止領域で、誤変更と手戻りを減らす
- テスト方法は「実行手順」と「期待結果」をセットで書く

#### 悪い例（情報不足）

```text
タイトル: 章8のIssue説明をAI向けに直してほしい
本文: いい感じに修正してください。必要ならテンプレも整えてください。
```

- どこまで直すか（受入基準）が不明
- 変更範囲（触ってよい/だめ）が不明
- テスト方法がなく、レビュー基準が揃わない

#### 良い例（実行仕様）

```markdown
タイトル: [Docs] 章8に「実行仕様Issue」テンプレと例題を追加する

## 背景 / 目的
AI支援でPull Requestを作る際に、Issueの情報不足で手戻りが発生しやすい。Issueを「実行仕様」として書くテンプレを章8に追加し、初心者が埋めて使える状態にする。

## 期待動作（Before / After）
- Before: 章8に、受入基準/制約/テスト方法まで含めた雛形がない
- After: 章8にテンプレ・良い例/悪い例・演習があり、Issueが仕様として成立する

## 受入基準
- [ ] 章8の「Issueの作成と管理」内に新しい節が追加されている
- [ ] テンプレに「受入基準」「制約・変更禁止領域」「テスト方法」が含まれている
- [ ] 良いIssue/悪いIssueの例題が1セットある
- [ ] 章末に「実行仕様Issueを書いてみる」演習が追加されている

## 制約・変更禁止領域
- 図（`assets/images/**`）の差し替えは行わない
- 既存の節番号は崩さず、追記のみで対応する

## テスト方法（最低限）
- 見出し構造（章番号・見出しレベル）が破綻していないことを確認する
- 章内リンクをクリックして意図した箇所に到達することを確認する
```
...
**カード**
- IssueやPull Requestをプロジェクトに追加すると、ボードビューではカードとして表示される
- ドラッグ&ドロップで簡単に移動可能
- 進捗に応じてカラム間を移動
...
**Step 3: Issueをプロジェクトに追加**

1. 既存のIssueから追加：Issue詳細画面で「Projects」を設定
2. プロジェクトから追加：プロジェクト画面の追加操作（`Add item` 等）でIssueを検索・選択
...
## 8.5 演習：実行仕様Issueを書いてみる

前述のテンプレを使い、実行仕様Issueを書いてみましょう（練習用リポジトリでも構いません）。テーマは、次のような「小さな変更」がおすすめです。

- 誤字/表現の改善
- リンク切れの修正
- 手順の補足（前提条件・注意事項・確認方法）

**チェック観点（自己レビュー）**
- 受入基準が検証可能になっている（Yes/Noで判定できる）
- 変更禁止領域が明記されている（触ってはいけない場所が分かる）
- テスト方法が第三者でも再現できる粒度になっている
- 機密情報・個人情報が含まれていない
